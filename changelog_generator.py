...
def generate_changelog(new_version: str):
    ...
    for branch, messages in branch_dict.items():
        ...
        branch_panel_lines = []
        if breaking:
            branch_panel_lines.append("⚠️ Breaking Changes:\n" + "\n".join(f"- {b}" for b in breaking))
        if features:
            branch_panel_lines.append("✨ Features:\n" + "\n".join(f"- {f}" for f in features))
        if fixes:
            branch_panel_lines.append("🐛 Fixes:\n" + "\n".join(f"- {fx}" for fx in fixes))

        panel_content = "\n\n".join(branch_panel_lines) if branch_panel_lines else "No notable changes."
        console.print(
            Panel(
                panel_content,
